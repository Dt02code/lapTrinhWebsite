...
// 2.10 Dữ liệu biểu đồ tròn - doanh thu theo ngày (7 ngày gần nhất)
$pieByDay = [];
$res = $conn->query(
    "SELECT DATE_FORMAT(h.NgayLap, '%d/%m') AS Label, SUM(h.TongTien) AS Revenue 
     FROM hoadon h 
     WHERE h.TrangThai = 'Đã giao' AND h.NgayLap >= DATE_SUB(CURRENT_DATE(), INTERVAL 7 DAY) 
     GROUP BY DATE(h.NgayLap), Label
     ORDER BY DATE(h.NgayLap) ASC"
);
if ($res) while ($r = $res->fetch_assoc()) $pieByDay[] = $r;

// 2.11 Dữ liệu biểu đồ tròn - doanh thu theo tháng (12 tháng gần nhất)
$pieByMonth = [];
$res = $conn->query(
    "SELECT DATE_FORMAT(h.NgayLap, 'Tháng %m/%Y') AS Label, SUM(h.TongTien) AS Revenue 
     FROM hoadon h 
     WHERE h.TrangThai = 'Đã giao' AND h.NgayLap >= DATE_SUB(CURRENT_DATE(), INTERVAL 12 MONTH) 
     GROUP BY DATE_FORMAT(h.NgayLap, '%Y-%m'), Label
     ORDER BY DATE_FORMAT(h.NgayLap, '%Y-%m') ASC"
);
if ($res) while ($r = $res->fetch_assoc()) $pieByMonth[] = $r;

// 2.12 Dữ liệu biểu đồ tròn - doanh thu theo năm
$pieByYear = [];
$res = $conn->query(
    "SELECT YEAR(h.NgayLap) AS Label, SUM(h.TongTien) AS Revenue 
     FROM hoadon h 
     WHERE h.TrangThai = 'Đã giao' 
     GROUP BY YEAR(h.NgayLap)
     ORDER BY YEAR(h.NgayLap) ASC"
);
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="tab-content">
            ...
            <div class="tab-pane fade" id="stats">
                ...
                <div class="row g-3 mb-4">
                    <div class="col-md-3">
                        <div class="card p-3" style="border-left:4px solid #3b82f6;">
                            <div class="d-flex align-items-center gap-3">
                                <div style="font-size:1.6rem;color:#3b82f6;"><i class="fa-solid fa-sack-dollar"></i></div>
                                <div>
                                    <div style="font-size:12px;color:#6b7280;">Tổng doanh thu</div>
                                    <div style="font-size:1.1rem;font-weight:700;color:#1e293b;"><?= number_format($totalRevenue, 0, ',', '.') ?>đ</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="card p-3" style="border-left:4px solid #10b981;">
                            <div class="d-flex align-items-center gap-3">
                                <div style="font-size:1.6rem;color:#10b981;"><i class="fa-solid fa-box-open"></i></div>
                                <div>
                                    <div style="font-size:12px;color:#6b7280;">Đơn đã giao</div>
                                    <div style="font-size:1.1rem;font-weight:700;color:#1e293b;"><?= $deliveredOrders ?> đơn</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="card p-3" style="border-left:4px solid #6366f1;">
                            <div class="d-flex align-items-center gap-3">
                                <div style="font-size:1.6rem;color:#6366f1;"><i class="fa-solid fa-cart-shopping"></i></div>
                                <div>
                                    <div style="font-size:12px;color:#6b7280;">Tổng đơn hàng</div>
                                    <div style="font-size:1.1rem;font-weight:700;color:#1e293b;"><?= $totalOrders ?> đơn</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="card p-3" style="border-left:4px solid #f59e0b;">
                            <div class="d-flex align-items-center gap-3">
                                <div style="font-size:1.6rem;color:#f59e0b;"><i class="fa-solid fa-triangle-exclamation"></i></div>
                                <div>
                                    <div style="font-size:12px;color:#6b7280;">Tồn kho thấp</div>
                                    <div style="font-size:1.1rem;font-weight:700;color:#1e293b;"><?= count($lowStock) ?> sản phẩm</div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card p-4 mb-4">
                    <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                        <h4 class="m-0"><i class="fa-solid fa-chart-pie text-primary me-2"></i>Biểu đồ tròn doanh thu</h4>
                        <div class="btn-group" role="group" id="pieFilterGroup">
                            <button type="button" class="btn btn-primary btn-sm pie-filter-btn active" data-filter="day"><i class="fa-solid fa-calendar-day me-1"></i>Theo ngày</button>
                            <button type="button" class="btn btn-outline-primary btn-sm pie-filter-btn" data-filter="month"><i class="fa-solid fa-calendar-week me-1"></i>Theo tháng</button>
                            <button type="button" class="btn btn-outline-primary btn-sm pie-filter-btn" data-filter="year"><i class="fa-solid fa-calendar me-1"></i>Theo năm</button>
                        </div>
                    </div>
                    <div class="row align-items-center">
                        <div class="col-lg-5 d-flex justify-content-center">
                            <div style="position:relative;width:280px;height:280px;">
                                ...
                                <div id="pieChartCenter" style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);text-align:center;pointer-events:none;">
                                    <div style="font-size:11px;color:#6b7280;">Tổng doanh thu</div>
                                    <div id="pieTotalLabel" style="font-size:15px;font-weight:700;color:#1e293b;">--</div>
                                </div>
                            </div>
                        </div>
                        <div class="col-lg-7">
                            <div id="pieLegend" class="pie-legend"></div>
                            <div id="pieNoData" class="alert alert-warning d-none">Chưa có dữ liệu doanh thu cho khoảng thời gian này.</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="row gy-4">
                    ...
                    <div class="col-lg-7">
                        <div class="card p-4">
                            <h5 class="mb-3"><i class="fa-solid fa-chart-bar text-success me-2"></i>Doanh thu 6 tháng gần nhất</h5>
                            <?php if (empty($revenueChart)): ?>
                                <div class="alert alert-warning">Chưa có doanh thu.</div>
                            <?php else: ?>
                                <canvas id="revenueBarChart" height="160"></canvas>
                            <?php endif; ?>
                        </div>
                        <div class="card p-4 mt-4">
                            <h5 class="mb-3"><i class="fa-solid fa-fire text-danger me-2"></i>Top sản phẩm bán chạy (tháng này)</h5>
                                ...
                                <?php else: ?>
                                    <?php foreach ($topProducts['month'] as $idx => $tp): ?>
                                        <li class="list-group-item d-flex justify-content-between align-items-center px-0">
                                            <span>
                                                <span class="badge me-2" style="background:<?= ['#667eea', '#f5576c', '#4facfe', '#43e97b', '#f093fb'][$idx] ?>;"><?= $idx + 1 ?></span>
                                                <?= htmlspecialchars($tp['TenSanPham']) ?>
                                            </span>
                                            <div class="text-end">
                                                <span class="badge bg-primary"><?= $tp['TotalSold'] ?> bán</span>
                                                <small class="d-block text-muted"><?= number_format($tp['TotalRev'], 0, ',', '.') ?>đ</small>
                                            </div>
                                        </li>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                    ...
                    <div class="col-lg-5">
                        <div class="card p-4">
                            <h5 class="mb-3"><i class="fa-solid fa-boxes-stacked text-warning me-2"></i>Sản phẩm tồn kho thấp</h5>
                            <?php if (empty($lowStock)): ?>
                            ...
                            <?php else: ?>
                                <ul class="list-group list-group-flush">
                                    <?php foreach ($lowStock as $item): ?>
                                        <li class="list-group-item d-flex justify-content-between align-items-center px-0">
                                            <div>
                                                <strong style="font-size:14px;"><?= htmlspecialchars($item['TenSanPham']) ?></strong><br>
                                                <small class="text-muted"><?= htmlspecialchars($item['DanhMuc']) ?></small>
                                            </div>
                                            <span class="badge bg-danger rounded-pill"><?= $item['SoLuong'] ?> còn</span>
                                        </li>
                                    <?php endforeach; ?>
                                </ul>
                            <?php endif; ?>
                        ...
                        <div class="card p-4 mt-4">
                            <h5 class="mb-3"><i class="fa-solid fa-list-check me-2 text-info"></i>Doanh thu 6 tháng (chi tiết)</h5>
                            <ul class="list-group list-group-flush">
                                <?php if (empty($revenueChart)): ?>
                                ...
                                <?php else: ?>
                                    <?php foreach ($revenueChart as $row): ?>
                                        <li class="list-group-item d-flex justify-content-between align-items-center px-0">
                                            <span class="text-muted" style="font-size:14px;"><?= htmlspecialchars($row['ChartMonth']) ?></span>
            ...
            <?php
            // Prepare JS data for pie charts
            $pieDataDay   = ['labels' => [], 'values' => []];
            foreach ($pieByDay   as $r) {
                $pieDataDay['labels'][]   = $r['Label'];
                $pieDataDay['values'][]   = (float)$r['Revenue'];
            }
            $pieDataMonth = ['labels' => [], 'values' => []];
            foreach ($pieByMonth as $r) {
                $pieDataMonth['labels'][] = $r['Label'];
                $pieDataMonth['values'][] = (float)$r['Revenue'];
            }
            $pieDataYear  = ['labels' => [], 'values' => []];
            foreach ($pieByYear  as $r) {
                $pieDataYear['labels'][]  = $r['Label'];
                $pieDataYear['values'][]  = (float)$r['Revenue'];
            }
            $revenueBarLabels = array_column($revenueChart, 'ChartMonth');
            $revenueBarValues = array_map(fn($r) => (float)$r['MonthRevenue'], $revenueChart);
            ?>
            <script>
                const PIE_DATA = {
                    day: <?= json_encode($pieDataDay,   JSON_UNESCAPED_UNICODE) ?>,
                    month: <?= json_encode($pieDataMonth, JSON_UNESCAPED_UNICODE) ?>,
                    year: <?= json_encode($pieDataYear,  JSON_UNESCAPED_UNICODE) ?>
                };
                const BAR_LABELS = <?= json_encode($revenueBarLabels, JSON_UNESCAPED_UNICODE) ?>;
                const BAR_VALUES = <?= json_encode($revenueBarValues) ?>;
            </script>
    ...
    <script>
        ...
        function switchTab(triggerEl) {
            // xóa thuộc tính active của tất cả các nav-link
    ...
    <script>
        (function() {
            const PALETTE = [
                ...
                '#8b5cf6', '#0ea5e9', '#14b8a6', '#f97316', '#84cc16',
                '#06b6d4', '#a855f7', '#64748b', '#ec4899', '#22d3ee'
            ];
            ...
            const pieTotalLabel = document.getElementById('pieTotalLabel');

            function buildPieLegend(labels, values, colors) {
                ...
                labels.forEach((lbl, i) => {
                    ...
                    <span class="pie-legend-value">${fmtVND(values[i])}</span>
            ...
            function updatePieChart(filter) {
                const data = PIE_DATA[filter];
                const labels = data.labels;
                const values = data.values;
                const total = values.reduce((a, b) => a + b, 0);

                pieTotalLabel.textContent = total > 0 ? fmtVND(total) : '0đ';
                ...
                buildPieLegend(labels, values, colors);
            ...
            // nút lọc biểu đồ tròn
            document.querySelectorAll('.pie-filter-btn').forEach(btn => {
                btn.addEventListener('click', function() {
                    document.querySelectorAll('.pie-filter-btn').forEach(b => {
                        b.classList.remove('active', 'btn-primary');
                        b.classList.add('btn-outline-primary');
                    });
                    this.classList.add('active', 'btn-primary');
                    this.classList.remove('btn-outline-primary');
                    updatePieChart(this.dataset.filter);
                });
            });

            // khỏi tạo biểu đòo tròn doanh thu
            const statsLink = document.querySelector('a[href="#stats"]');
            if (statsLink) {
                statsLink.addEventListener('click', function() {
                    setTimeout(() => {
                        if (!pieChart) updatePieChart('day');
                    }, 120);
                });
            }
            ...
            if (barCanvas && typeof BAR_LABELS !== 'undefined' && BAR_LABELS.length) {
                new Chart(barCanvas, {
                    type: 'bar',
                    data: {
                        labels: BAR_LABELS,
                        datasets: [{
                            label: 'Doanh thu',
                            data: BAR_VALUES,
                            backgroundColor: BAR_VALUES.map((_, i) => PALETTE[i % PALETTE.length] + 'cc'),
                            borderColor: BAR_VALUES.map((_, i) => PALETTE[i % PALETTE.length]),
                            borderWidth: 2,
                            borderRadius: 6
                        }]
                    },
                    options: {
                        ...
                        plugins: {
                            ...
                            tooltip: {
                                callbacks: {
                                    label: ctx => ' ' + Number(ctx.parsed.y).toLocaleString('vi') + 'đ'
                                }
                            }
                        },
                        scales: {
